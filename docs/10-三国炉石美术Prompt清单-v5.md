# 三国炉石 v5 · 全美术资源 Prompt 清单（v5.7 蜀士卒重组 + 吴效果互换版）
...
### ⛔ #22 · ~~绵羊 token~~（v5.5 已删除：W06 吕蒙战吼改「暗度陈仓」下回合 +3 mana 破 cap，不再变形）

旧绵羊 token 视觉条目作废，无需任何立绘。

---

### #5f · S08 屯田（蜀法术·setup） · `tuntian.png`

- **像素**：1086×1448 · **背景**：实色 · **稀有度**：普通 · **对应卡 ID**：S08 🆕 v5.5 新增
- **机制**：1 费蜀法术。下一回合开始时，当回合法力 +2（仅下回合，不影响下下回合上限）
- **设计意图**：补蜀低费曲线 setup；不破隔离规则（不是直接抽牌，是 mana setup 类似 W17 借东风）
- ✅ **用户已切图** · `asset/新增ui组件0605/边框+立绘/屯田.png`

---

### #5g · S09 魏延（蜀史诗武将） · `weiyan.png`

- **像素**：1086×1448 · **背景**：实色 · **稀有度**：史诗 · **对应卡 ID**：S09 🆕 v5.5 新增
- **机制**：5 费史诗武将 5/4 突袭。战吼：使一友方武将获得 +2 攻击力（永久）
- **设计意图**：补蜀史诗武将曲线 + 滚雪球加速器（配关羽/张飞/赵云爆发更强）
- ✅ **用户已切图** · `asset/新增ui组件0605/边框+立绘/魏延.png`

---

### ⛔ #5h · ~~W06 吕蒙战吼立绘 暗度陈仓.png~~（v5.5 作废）

- v5.5 拍板：W06 吕蒙战吼"暗度陈仓"不做全屏弹出动作，不需独立立绘
- 改用 **CSS 微特效**：mana badge 蓝色闪光 0.5s + 卡牌微抖动 + 下回合开始时大字 +3 弹出
- 用户已画的 `暗度陈仓.png` 不接入游戏，仅留作参考

---

### ⛔ 旧 #20 医匠 token · 木牛流马 token

v5.5 改名为「木牛流马」，立绘文件名沿用 `token_yijiang.png` 不动（仅卡名/Token ID 改 TK_MUNIULIUMA）。立绘已用户提供。
...
## 四、法术立绘（18 张，v5.3 重构：+1 画地为牢 / -1 火攻连环 / 净持平）
...
### #5c · W22 春风化雨（吴治疗/防御·大乔锚点）⭐ v5.5 互换效果 · `chunfenghuayu.png`

- **像素**：1086×1448 · **背景**：实色 · **稀有度**：普通 · **对应卡 ID**：W22（v5.5 互换：原 W16 固守待援的治疗+群增益效果迁移到 W22 春风化雨）
- **机制**：3 费恢复主公 **6 HP**；大乔在场（`anchor_heal`）+ **3 HP（共 9）** + 全友方武将 +0/+1
- **设计意图**：v5.5 W16/W22 名字归位置，效果跟名字含义匹配。「春风化雨」温柔强大 = 治疗+群增益；anchor_heal 锚点联动跟机制不跟名字
- 🟡 **prompt 待用户写**（按 docs/10 风格 checklist + 共享头模板）

---

### #5e · W16 固守待援（吴控场工具卡·无锚点）⭐ v5.5 互换为发现卡 · `gushoudaiyuan.png`

- **像素**：1086×1448 · **背景**：实色 · **稀有度**：普通 · **对应卡 ID**：W16（v5.5 互换：原 W22 二乔的发现谋略效果迁移到 W16 固守待援）
- **机制**：**2 费**（v5.5 由 3 费降）；**发现 1 张 ≤4 费谋略卡**（HS 3 选 1）
- **设计意图**：v5.5 W16/W22 名字归位置。「固守待援」中性工具 = 发现谋略；无锚点联动
- 🟡 **prompt 待用户写**（按 docs/10 风格 checklist + 共享头模板）

---

### #5d · W26 画地为牢（吴软控场·无锚点）⭐ v5.3 新增（接替 W10 火攻连环槽位） · `huadiweilao.png`
...
### #24 · 万军取首（蜀法术）⭐ v5.5 改名+加 stat（原 五虎合击） · `wanjunqushou.png`

- **像素**：1086×1448 · **背景**：实色 · **稀有度**：传说 · **对应卡 ID**：S22
- **机制**：5 费传说法术。使一友方武将获得 **+1/+1**，本回合再攻击 1 次（v5.5 由「五虎合击」改名 + 加 stat buff）
- **视觉**：单骑斩将（替代旧"五虎合围"群斗视觉）
- ✅ **用户已切图** · `asset/新增ui组件0605/边框+立绘/万军取首.png`
...
| A4 | 法术立绘 | 18 | 0 | 18（v5.3 重构：+1 画地为牢 / -1 火攻连环 / 净持平；含 4 改名 + 数值削弱）| `<英文名>.png` 放 `portraits/` |
...
**v5.4 卡牌重构净持平**：法术删 W10 火攻连环 + 新增 W26 画地为牢 = 净 0；中立删 N01 谋士（无独立 prompt） + S18 关羽去武器附赠（资源已为 legacy 18 张复用，不动新 prompt 数）。删除卡的章节（#26/#27/#28/#34）保留为「已删除」空壳作为变更追溯。
...
- **v5.7（2026-06-09 v5.5 蜀士卒重组 + 吴效果互换）**：① S22 五虎合击→万军取首（#24 重写，加 +1/+1 buff）；② W16/W22 效果互换（#5c 改 W22 春风化雨治疗+群增益 / 新增 #5e W16 固守待援发现卡）；③ W26 缓兵之计→画地为牢（撤回 v5.3 改名，文件名 huadiweilao.png）；④ 新增 #5f S08 屯田 / #5g S09 魏延；⑤ 删 #22 绵羊 token（吕蒙不变形）；⑥ 删 #5h 暗度陈仓立绘（改 CSS 微特效）；⑦ S13/S14 章节不再独立（已并入"已删除"卡列表）
...
- **v5.4（2026-06-05 用户卡牌重构）**：删 W10 火攻连环 prompt 块（#26）；删 W16 春风化雨 prompt 块（#34，改名为固守待援移至 #5c）；新增 #5c 固守待援 / #5d 画地为牢 / W11 改名 行舟借势 / W12 改名+降费 周郎顾曲；TK_YIJIANG → 机关木甲 token 章节标题改名；删除 TK_QINGLONG token（关羽去武器附赠后无召唤源）；#40 金疮药 数值更新（2 费回 5）
