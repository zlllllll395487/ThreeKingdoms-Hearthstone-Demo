...
> **最后更新**：2026-06-17 · 对应 commit `6d5a654`
>
...
| 战斗内自动托管 + 取消托管提示 | ✓ |
| 战后结算 → 返回主菜单 | ✓ |
| **在线真人对战**（房间码约战 · 权威服务器 · 出牌/攻击/结束回合同步 · 胜负 · 对手离开处理） | ✓ |
...
| 公开匹配（随机配对） | 当前仅房间码约战；架构已留接口，待后续 |
| 在线对战的效果文字 toast / 回合记录 | log 涉及脱敏未下发（`logDraw` 会泄露对手抽牌名）；伤害浮字等状态驱动反馈正常 |
| 在线部署形态（7×24 云端） | 当前本地后端 + 内网穿透（ngrok）做 MVP，未迁云 |
